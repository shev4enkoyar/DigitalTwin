...
class CreateModel extends Component {
    constructor(props) {
        super(props);
        this.state = { children: [{ name: "", pred: "Да", visible: true }, { hist: false, visible: false }, { cult: 0, sort: 0, frac: "", gust: "", norm: "", total: "", visible: false }, { kad: "", visible: false }, { added: true, visible: false }, { chem: true, visible: false}, { dat: false, visible: false }, { econom: true, visible: false }] };
    }
            ...
            <div className="d-flex justify-content-center w-100 h-100 position-absolute">
                <Container className="d-flex p-0 m-0" style={{ backgroundColor: '#302F38', borderRadius: '3px', border: '1px solid #000', maxHeight: '65px', }}>{
                    this.steps.map((value, index) =>
                        <ElementStep active={this.state.children[index].visible} progress={this.handleIsProgress()>=index} numberStep={index + 1} nameCard={value} />
                    )}
                </Container>
                ...
                <EconomicCard setStatus={(v) => { this.setStatus(v, 7) }} values={this.state.children[7]} visible={this.state.children[7].visible} allValues={this.state}/>
            </div>
        );
    }
